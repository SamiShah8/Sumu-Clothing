      ...
      <div className="container--fluid">
        <div className="flex justify-between items-center py-4 md:hidden ">
          <div className="border border-[--var(btn-color)]-500 rounded-lg md:hidden hover:border-[var(--secondary-color)]  sm:hidden">
            <GiHamburgerMenu
              onClick={showMenu}
              className={`h-[2rem] w-[2rem] p-2 cursor-pointer ${showMobileNav ? "hidden" : "block"}`}
            />
            <ImCross onClick={hideMenu} className={`h-[2rem] w-[2rem] p-2 cursor-pointer ${showMobileNav ? "block" : "hidden"}   `} />
          </div>
        ...
        <nav className={`mt-1 ${showMobileNav ? "block" : "hidden"} md:hidden`}>
          <div className="text-[0.8rem] ">
            ...
            <ul className="flex flex-col pt-[0.25rem] gap-1 cursor-pointer pl-[0.4rem] ">
              ...
              <li>Stikers</li>
              <li>
                <button className="border border-[--var(btn-color)]-600 rounded-lg  p-1 text-[0.6rem] font-extrabold">
                  Hello, Signin
                </button>
              </li>
            </ul>
